...
async def get_reports_data(period: str = '3m', db: Session = Depends(get_db)):
    ...
    # --- 1. Zestawienie Finansowe (Przychód, Koszt, Zysk) ---
    monthly_financials = defaultdict(lambda: {'revenue': 0, 'costs': 0})


    revenue_query = db.query(
        func.to_char(services_models.Service.date, 'YYYY-MM').label('month'),
    ...
    costs_query = db.query(
        func.to_char(costs_models.Expense.invoice_date, 'YYYY-MM').label('month'),
    ...
    expense_query = db.query(costs_models.ExpenseCategory.name, func.sum(costs_models.Expense.amount_gross)).join(costs_models.ExpenseCategory)
    if start_date:
    ...
    revenue_query = db.query(services_models.Category.name, func.sum(services_models.Service.base_price_net)).select_from(services_models.Service).join(services_models.ServiceType).join(services_models.Category)
    if start_date:
        revenue_query = revenue_query.filter(services_models.Service.date >= start_date)
    revenue_structure_data = revenue_query.group_by(services_models.Category.name).all()

    popularity_query = db.query(services_models.ServiceType.name, func.count(services_models.Service.id)).join(services_models.ServiceType)
    if start_date:
        popularity_query = popularity_query.filter(services_models.Service.date >= start_date)
    service_popularity_data = popularity_query.group_by(services_models.ServiceType.name).order_by(desc(func.count(services_models.Service.id))).all()

    source_query = db.query(clients_models.Client.source, func.count(clients_models.Client.id)).filter(clients_models.Client.source.isnot(None))
    if start_date:
    ...
    future_appointments = db.query(calendar_models.Appointment).join(services_models.ServiceType).join(services_models.Category).filter(
    ...
    forecasted_revenue_data = { "labels": sorted_revenue_months, "data": [forecast_revenue_by_month[m] for m in sorted_revenue_months] }

    sorted_services = sorted(forecast_services_by_type.items(), key=lambda item: item[1], reverse=True)[:10]
    forecasted_services_data = { "labels": [item[0] for item in sorted_services], "data": [item[1] for item in sorted_services] }

    all_months = sorted(list(set(month for categories in forecast_trend_by_category_and_month.values() for month in categories.keys())))
    ...
    for category_name in all_categories:
        ...
        trend_datasets.append({
            "label": category_name, "data": counts, "borderColor": color,
            "backgroundColor": color.replace(')', ', 0.1)').replace('rgb', 'rgba'),
            "fill": False, "tension": 0.2
        })
    forecasted_service_count_data = { "labels": all_months, "datasets": trend_datasets }

    return {
        "financial_summary": financial_summary_data,
        "expense_breakdown": {"labels": [r[0] for r in expense_breakdown_data], "data": [r[1] for r in expense_breakdown_data]},
        "revenue_structure": {"labels": [r[0] for r in revenue_structure_data], "data": [r[1] for r in revenue_structure_data]},
        "service_popularity": {"labels": [r[0] for r in service_popularity_data], "data": [r[1] for r in service_popularity_data]},
